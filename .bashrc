...
alias noed='f() { clasp pull && clasp push --watch git add -f "$1" && cloudshell edit "$1" && $npxc prettier --watch --html-whitespace-sensitivity "ignore" "$1" &&  git commit -m "$1" && git push && git stash --keep-index --include-untracked; unset -f f; }; f'     
