...
alias gwsta='f() { git stash push -u -m "WIP: My local changes before clasp pull" && clasp pull && git add . && git commit -m "Pulled latest changes from Apps Script editor" && git stash apply && git add . && git commit -m "Applied stashed changes for new feature"; unset -f f; }; f'
