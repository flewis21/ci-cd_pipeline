...
alias noed='f() { gwsta && cloudshell edit "$1"; unset -f f; }; f'
...
alias gwsta='f() { git stash push -u -m "WIP: My local changes before clasp pull" && clasp pull && git add -A && git commit -m "Pulled latest changes from Apps Script editor" && git stash apply && git add -A && git commit -m "Applied stashed changes for new feature"; unset -f f; }; f'
