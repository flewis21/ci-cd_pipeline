...
class LinuxClipboard extends BaseClipboard {
    ...
    async copyImage(imageFile) {
        ...
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_set_clipboard_png.sh");
    ...
    async copyTextPlain(textFile) {
        ...
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_set_clipboard_text_plain.sh");
    ...
    async copyTextHtml(htmlFile) {
        ...
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_set_clipboard_text_html.sh");
    ...
    async getContentType() {
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_get_clipboard_content_type.sh");
        try {
            const shell = getShell();
    ...
    async getImage(imagePath) {
        if (!imagePath)
            return "";
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_save_clipboard_png.sh");
        const shell = getShell();
        const data = await shell.runScript(script, [imagePath]);
        return stripFinalNewline(data);
    }
    async getTextPlain() {
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_get_clipboard_text_plain.sh");
        const shell = getShell();
        const data = await shell.runScript(script);
        return stripFinalNewline(data);
    }
    async getTextHtml() {
        const script = path.join(__dirname, this.SCRIPT_PATH, "linux_get_clipboard_text_html.sh");
        const shell = getShell();
        const data = await shell.runScript(script);
        return stripFinalNewline(data);
    }
